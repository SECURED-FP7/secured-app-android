...
{
	SoupMessage *message;//richiesta e risposta HTTP
	...
	if (this->type)
	{
		soup_message_set_request(message, this->type, SOUP_MEMORY_STATIC,
								 this->data.ptr, this->data.len);//setta il body del messaggio
	}
	soup_message_set_http_version(message, this->version);//setta la versione HTTP del messaggio
	soup_message_body_set_accumulate(message->response_body, FALSE);
	g_signal_connect(message, "got-chunk", G_CALLBACK(soup_cb), &data);
	data.session = soup_session_sync_new();//crea una SoupSession sincrona senza argomenti particolari
	g_object_set(G_OBJECT(data.session),
				 SOUP_SESSION_TIMEOUT, (guint)this->timeout, NULL);

	DBG1(DBG_LIB, "sending http request to '%s'...", uri);
	soup_session_send_message(data.session, message);//funzione sincrona che accumula tutto il response body
