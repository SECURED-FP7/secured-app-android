...
static void load_feature(private_plugin_loader_t *this,
						 ...
						 int level)
{
	//dbg(DBG_LIB, 1, "PROVO A CARICARE %s", provided->entry->plugin->get_name(provided->entry->plugin));
	...
	//dbg(DBG_LIB, 1, "ATTUALMENTE NON HO CARICATO %s", provided->entry->plugin->get_name(provided->entry->plugin));
	...
	private_plugin_loader_t *this, char *list)
{
	...
	plugins = modular_pluginlist(list);//list è la lista dei plugin da caricare
	//dbg(DBG_LIB, 1, "LISTA DEI PLUGIN DA CARICARE: %s", list);//ma in realtà il programma fa di testa sua
