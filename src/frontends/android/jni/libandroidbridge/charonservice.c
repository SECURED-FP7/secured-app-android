...
static void dbg_android(debug_t group, level_t level, char *fmt, ...) {
	...
	if (level <= ANDROID_DEBUG_LEVEL) {
		...
		va_end(args);
		while (current) { /* log each line separately */
			next = strchr(current, '\n');
			if (next) {
				*(next++) = '\0';
			}
...
METHOD(charonservice_t, update_status, bool, private_charonservice_t *this,
		android_vpn_state_t code) {
	JNIEnv *env;
	jmethodID method_id;
	bool success = FALSE;
	...
	failed: androidjni_exception_occurred(env);
	androidjni_detach_thread();
	return success;
}

METHOD(charonservice_t, update_imc_state, bool, private_charonservice_t *this,
		android_imc_state_t state) {
	JNIEnv *env;
	jmethodID method_id;
	bool success = FALSE;
...
METHOD(charonservice_t, add_remediation_instr, bool,
	...
	method_id = (*env)->GetMethodID(env, android_charonvpnservice_class,
			"addRemediationInstruction", "(Ljava/lang/String;)V");
	if (!method_id) {
		goto failed;
	}
...
METHOD(charonservice_t, bypass_socket, bool, private_charonservice_t *this,
		int fd, int family) {
	if (fd >= 0) {
...
static void initiate(char *type, char *gateway, char *username, char *password) {
	...
	this->service = android_service_create(this->creds, type, gateway, username,
			password);
}
...
static bool charonservice_register(plugin_t *plugin, plugin_feature_t *feature,
	...
	if (reg) {
		...
		hydra->attributes->add_handler(hydra->attributes, &this->attr->handler);
	} else {
...
static void set_options(char *logfile) {
	//INIZIO MIE AGGIUNTE
	//Setto i parametri che saranno richiamati da oat_attest_listener.c
	char c;
	FILE* file;
	lib->settings->set_str(lib->settings, "%s.plugins.oat-attest.ima_level",
			"l4_ima_all_ok", lib->ns); //grado di affidabilità

	//lib->settings->set_str(lib->settings, "%s.plugins.oat-attest.request_id",
//			"101", lib->ns);
	//lib->settings->set_bool(lib->settings, "%s.plugins.oat-attest.attest_peer",
	//		TRUE, lib->ns); //se false non fa attestazione
	lib->settings->set_int(lib->settings,
			"%s.plugins.oat-attest.attest_interval", 15, lib->ns); //nuova aggiunta di Tao

	file = fopen("/data/data/org.strongswan.android/cache/flagRA.txt", "r");
	...
	if (file != NULL) {
		...
		if (c == 'v') {
			lib->settings->set_bool(lib->settings,
					"%s.plugins.oat-attest.attest_peer", TRUE, lib->ns); //se true fa attestazione
			...
			if (file2 != NULL) {
				const int buzz_size = 30;
				char buff[buzz_size];
				fgets(buff, buzz_size, file2);
				fclose(file2);
				lib->settings->set_str(lib->settings,
						"%s.plugins.oat-attest.oat_server", buff, lib->ns); //server verificatore già conosciuto

			} else {
				dbg_android(DBG_LIB, 1, "The verifier IP was not inserted");
			}
		} else {
			lib->settings->set_bool(lib->settings,
					"%s.plugins.oat-attest.attest_peer", FALSE, lib->ns); //se false non fa attestazione

		}

	}

	dbg_android(DBG_LIB, 1, "AGGIUNTE MIE");

	//FINE AGGIUNTE MIE

	lib->settings->set_int(lib->settings, "charon.plugins.android_log.loglevel",
			ANDROID_DEBUG_LEVEL);
	/* setup file logger */
	lib->settings->set_str(lib->settings, "charon.filelog.%s.time_format",
			"%b %e %T", logfile);
	...
	lib->settings->set_bool(lib->settings, "charon.filelog.%s.flush_line", TRUE,
			logfile);
	lib->settings->set_int(lib->settings, "charon.filelog.%s.default",
			ANDROID_DEBUG_LEVEL, logfile);

	lib->settings->set_int(lib->settings, "charon.retransmit_tries",
			ANDROID_RETRASNMIT_TRIES);
	lib->settings->set_double(lib->settings, "charon.retransmit_timeout",
			ANDROID_RETRANSMIT_TIMEOUT);
	lib->settings->set_double(lib->settings, "charon.retransmit_base",
			ANDROID_RETRANSMIT_BASE);
	lib->settings->set_bool(lib->settings, "charon.close_ike_on_child_failure",
			TRUE);
	...
	lib->settings->set_bool(lib->settings, "charon.install_virtual_ip", FALSE);
	...
	lib->settings->set_str(lib->settings, "charon.interfaces_ignore",
			"lo, tun0, tun1, tun2, tun3, "
					"tun4");
...
static void charonservice_init(JNIEnv *env, jobject service, jobject builder,
		jboolean byod) {
	private_charonservice_t *this;
	static plugin_feature_t features[] = { PLUGIN_CALLBACK(
			kernel_ipsec_register, kernel_android_ipsec_create), PLUGIN_PROVIDE(
			CUSTOM, "kernel-ipsec"), PLUGIN_CALLBACK(charonservice_register,
			NULL), PLUGIN_PROVIDE(CUSTOM, "android-backend"), PLUGIN_DEPENDS(
			CUSTOM, "libcharon"), };

	INIT(this,
			.public = {.update_status = _update_status, .update_imc_state = _update_imc_state, .add_remediation_instr = _add_remediation_instr, .bypass_socket = _bypass_socket, .get_trusted_certificates = _get_trusted_certificates, .get_user_certificate = _get_user_certificate, .get_user_key = _get_user_key, .get_vpnservice_builder = _get_vpnservice_builder, .get_network_manager = _get_network_manager, }, .attr = android_attr_create(), .creds = android_creds_create(), .builder = vpnservice_builder_create(builder), .network_manager = network_manager_create(service), .sockets = linked_list_create(), .vpn_service = (*env)->NewGlobalRef(env, service), );
	charonservice = &this->public;

	lib->plugins->add_static_features(lib->plugins, "androidbridge", features,
			countof(features), TRUE);
	/*
	 DBG1(DBG_DMN, "AGGIUNTA STATICA DI SOUP");
	 private_soup_plugin_t *sp;
	 plugin_feature_t f[] = {
	 PLUGIN_REGISTER(FETCHER,sp-> soup_fetcher_create),
	 PLUGIN_PROVIDE(FETCHER, "http://"),
	 PLUGIN_PROVIDE(FETCHER, "https://"),
	 };

	 lib->plugins->add_static_features(lib->plugins, "soup", f,
	 countof(f), TRUE);*/
// FINE AGGIUNTA STATICA DI SOUP
#ifdef USE_BYOD
	if (byod)
	{
