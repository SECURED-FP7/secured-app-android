			...
			synchronized (this)
			{
				try
				{
					...
					else
					{
						...
						//inizializzo il sender per le notifiche circa l'esito dell'istaurazione del tunnel
						NotificationBlockingSender.getInstance().InitiateNotificationSender_forTrafficBlock(getBaseContext());
						...
						if (initializeCharon(builder, mLogFile, mCurrentProfile.getVpnType().getEnableBYOD()))
						{
							...
							Log.i(TAG, "MONITORO CAMBIAMENTI");
							//inizio il servizio per monitorare i cambiamenti alla rete
							startService(new Intent(CharonVpnService.this, org.strongswan.android.netgroup.secured.Network_monitor_service.class));
						}
						else
						{
	...
	public void updateStatus(int status)
	{
		switch (status)
		{
			...
			case STATE_CHILD_SA_UP:
				...
				//aggiunta mia per lanciare il browser sull'indirizzo del default gateway
				WifiManager manager = (WifiManager) getSystemService("wifi");
				DhcpInfo dhcp = manager.getDhcpInfo();
				Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://"+intToIp(dhcp.gateway)));
