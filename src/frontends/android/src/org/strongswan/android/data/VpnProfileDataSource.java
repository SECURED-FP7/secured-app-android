...
public class VpnProfileDataSource {
	...
	public static final String KEY_REMOTE_ATTESTATION = "remote_attestation";
	...
	private VpnProfile VpnProfileFromCursor(Cursor cursor) {
		...
		if (cursor.getString(cursor.getColumnIndex(KEY_REMOTE_ATTESTATION)).equals("TRUE"))
			profile.setRA(true);
		else
			profile.setRA(false);
		return profile;
	...
	private ContentValues ContentValuesFromVpnProfile(VpnProfile profile) {
		...
		if (profile.getRA()==true) values.put(KEY_REMOTE_ATTESTATION, "TRUE");
		else values.put(KEY_REMOTE_ATTESTATION, "FALSE");
		return values;
	}
}
