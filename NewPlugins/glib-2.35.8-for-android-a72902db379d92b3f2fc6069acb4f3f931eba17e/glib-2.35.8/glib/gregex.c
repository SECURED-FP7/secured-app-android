             ...
             GError             **error)
{
  ...
  if (g_once_init_enter (&initialised))
    ...
    LOGD("== PCRE library is compiled without UTF8 support\n");
    LOGD("== PCRE library is compiled without UTF8 support\n");	
      	}
      ...
      if (!supports_ucp) {
        g_critical (_("PCRE library is compiled without UTF8 properties support"));
    		LOGD("== PCRE library is compiled without UTF8 properties support\n");
	    	LOGD("== PCRE library is compiled without UTF8 properties support\n");
      	}

      g_once_init_leave (&initialised, supports_utf8 && supports_ucp ? 1 : 2);
    } else {
        LOGD("== g_once_init_enter false\n");
        LOGD("== g_once_init_enter false\n");
    }
